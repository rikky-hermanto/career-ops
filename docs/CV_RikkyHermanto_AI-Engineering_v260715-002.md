...
A self-built platform that parses Indonesian bank statements with LLMs and gamifies personal finance into a scoring compass toward financial freedom. The AI layer runs against my own real financial data (4,400+ transactions), so every feature is evaluated against live data — not assumed to work. Current direction: evolving it into an agentic financial advisor — tool-calling agents with traceable reasoning, stateful multi-step workflows (LangGraph), and an MCP server exposing the finance data as tools.
